...
description: "Fix missing commands, incorrect SDK selection, and editor setup."
---
...
If the executable is missing, [install FVM](/#installation) again.
...
If `fvm flutter --version` is correct but `flutter --version` is not, repeat [Shell Setup](/#shell-setup). The FVM launcher must come first on PATH.
...
Run `fvm use <version>` again and set the editor's Flutter SDK path to `.fvm/flutter_sdk` beside `.fvmrc`. Restart the editor. See [Editor setup](/#editor-setup) for the VS Code setting.
...
## Using standalone Dart or another manager

Use `fvm dart --version` to check the Dart bundled with your project's Flutter SDK. Plain `dart` may use a separate installation, including DVM. `.dvmrc` selects standalone Dart; `.fvmrc` selects Flutter. Use `fvm exec <command>` for scripts that need both Flutter and its Dart on PATH.

These docs describe `mrgnhnt96/fvm`, which is separate from the pub.dev package named `fvm`. If you keep both managers, choose which executable PATH should find and give this one a separate `FVM_HOME`. Existing SDK caches are not imported; install the versions you need with `fvm install`.
